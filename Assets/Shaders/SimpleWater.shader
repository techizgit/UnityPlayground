	...
	SubShader
	{
        ...
        Pass
        {
            ...
            vertexOutput vert(vertexInput input)
            {
                ...
                output.grabPos.y += _CameraDepthTexture_TexelSize.z * abs(_CameraDepthTexture_TexelSize.y);
                output.grabPos.x += 1;
            ...
            float4 frag(vertexOutput input) : COLOR
            {
            	...
            	float backgroundDepth = LinearEyeDepth(SAMPLE_DEPTH_TEXTURE_PROJ(_CameraDepthTexture, input.grabPos));
            	float surfaceDepth = UNITY_Z_0_FAR_FROM_CLIPSPACE(input.grabPos.z);
            	float depthDifference = backgroundDepth - surfaceDepth;

                return tex2Dproj(_BackgroundTexture , input.grabPos) * saturate((1-depthDifference/15));
            }
            ENDCG
        }
